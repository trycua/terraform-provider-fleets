...
func validateMapping(schemas crdSchemas, config mapping) {
	...
	for _, item := range config.Blocks {
		registerTopLevel(item.Name, item.GoName)
		if item.Collection != "single" && item.Collection != "set" && item.Collection != "list" {
...
func validateFieldConfig(item field) {
	...
	case "string", "int64", "bool", "normalized_json", "string_list", "int64_list", "string_map":
	default:
...
func validateField(node map[string]any, item field) {
	...
	info := inspect(node)
	expectedType, elementType := crdTypes(item.ValueType)
	...
	if expectedType == "array" {
		...
		element, _ = node["additionalProperties"].(map[string]any)
	}
	if actual, _ := element["type"].(string); actual != elementType {
		fatalf("field %s maps to CRD element type %q, expected %q", item.Name, actual, elementType)
	}
}

// crdTypes is the CRD type a Terraform value type maps to and, for
// collections, the CRD type of its elements.
func crdTypes(valueType string) (string, string) {
	switch valueType {
	case "int64":
		return "integer", ""
	case "bool":
...
func render(schemas crdSchemas, config mapping) []byte {
	...
	file.WriteString("// Code generated by cmd/generate-pool-resource; DO NOT EDIT.\n\n")
	file.WriteString("package provider\n\n")
	file.WriteString("import (\n\t\"regexp\"\n\n")
	body := out.String()
	for _, item := range generatedImports {
		// Import only what the body uses so new value types never leave an
		// unused import behind.
		if strings.Contains(body, item.selector+".") {
			fmt.Fprintf(&file, "\t%q\n", item.path)
		}
	}
	file.WriteString(")\n\n")
...
var generatedImports = []struct{ path, selector string }{
	{"github.com/hashicorp/terraform-plugin-framework-jsontypes/jsontypes", "jsontypes"},
	{"github.com/hashicorp/terraform-plugin-framework-validators/int64validator", "int64validator"},
	{"github.com/hashicorp/terraform-plugin-framework-validators/listvalidator", "listvalidator"},
	{"github.com/hashicorp/terraform-plugin-framework-validators/stringvalidator", "stringvalidator"},
	{"github.com/hashicorp/terraform-plugin-framework/attr", "attr"},
	...
	{"github.com/hashicorp/terraform-plugin-framework/schema/validator", "validator"},
...
func renderAttribute(out *bytes.Buffer, node map[string]any, item field, indent string) {
	...
	if element := elementType(item.ValueType); element != "" {
		fmt.Fprintf(out, ", ElementType: %s", element)
	}
	if description != "" {
...
func validatorsFor(item field, info schemaInfo) []string {
	...
	if item.ValueType == "int64_list" && info.Items != nil {
		if bounds := int64Bounds(*info.Items); bounds != "" {
			result = append(result, "listvalidator.ValueInt64sAre("+bounds+")")
		}
	}
...
func int64Bounds(info schemaInfo) string {
	switch {
	case info.Minimum != nil && info.Maximum != nil:
		return fmt.Sprintf("int64validator.Between(%d, %d)", *info.Minimum, *info.Maximum)
	case info.Minimum != nil:
		return fmt.Sprintf("int64validator.AtLeast(%d)", *info.Minimum)
	default:
		return ""
	}
}
...
func modelType(valueType string) string {
	...
	return "types." + title(baseType(valueType))
}
...
func attrType(valueType string) string {
	...
	case "string_list", "int64_list":
		return "types.ListType{ElemType: " + elementType(valueType) + "}"
	case "string_map":
		return "types.MapType{ElemType: " + elementType(valueType) + "}"
	default:
		return "types.StringType"
	}
}

// elementType is the element attr.Type of a collection value type, or "".
func elementType(valueType string) string {
	switch valueType {
	case "string_list", "string_map":
		return "types.StringType"
	case "int64_list":
		return "types.Int64Type"
	default:
...
// baseType is the Terraform schema kind of a value type.
